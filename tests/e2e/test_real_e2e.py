#!/usr/bin/env python3
"""
Real End-to-End Test for Completion Detection System

This script tests the actual completion detection system with the real database schema.
"""
...
def create_test_plan(conn, num_tasks: int = 3) -> str:
    ...
    try:
        ...
        # Insert plan (using actual schema)
        cursor.execute("""
            INSERT INTO plans (id, name, tasks, status, created_at)
            VALUES (?, ?, ?, ?, ?)
        """, (plan_id, f"E2E Test Plan {plan_id}", json.dumps(tasks),
...
def add_plan_log(conn, plan_id: str, task_id: str, message: str):
    """Add a plan log to simulate task completion"""
    cursor = conn.cursor()
...
def get_plan_logs(conn, plan_id: str) -> list:
    """Get all logs for a plan"""
    cursor = conn.cursor()

    try:
        cursor.execute("SELECT * FROM plan_logs WHERE plan_id = ?", (plan_id,))
...
def get_task_count(conn, plan_id: str) -> int:
    """Get number of tasks in a plan"""
    cursor = conn.cursor()

    try:
        cursor.execute("SELECT tasks FROM plans WHERE id = ?", (plan_id,))
        row = cursor.fetchone()
        if row and row[0]:
...
def check_completion_pattern(log_message: str) -> bool:
    """Check if log message contains completion pattern"""
    completion_patterns = [
        "✔ finished — end_turn",
        "task completed:",
        "✓ done",
    ...
    return any(pattern.lower() in log_lower for pattern in completion_patterns)
...
def check_completion(conn, plan_id: str) -> dict:
    ...
    try:
        # Get total tasks
        total_tasks = get_task_count(conn, plan_id)

        # Get all logs
        logs = get_plan_logs(conn, plan_id)

        # Count completion logs
        completion_logs = sum(1 for log in logs if check_completion_pattern(log['message']))

        # Check if all tasks have completion indicators
        should_complete = completion_logs >= total_tasks if total_tasks > 0 else False
...
def test_1_happy_path(conn) -> dict:
    ...
    try:
        ...
        for i in range(1, 4):
            time.sleep(0.5)
            add_plan_log(conn, plan_id, f"task-{i}", f"✔ finished — end_turn for task-{i}")
        ...
        if check_result['should_complete']:
            ...
            if plan['status'] == 'success':
                ...
                result["notes"].append(f"✗ Plan status is '{plan['status']}' instead of 'success'")
        else:
            result["status"] = "failed"
...
def test_2_partial_completion(conn) -> dict:
    ...
    try:
        ...
        for i in range(1, 4):
            time.sleep(0.5)
            add_plan_log(conn, plan_id, f"task-{i}", f"Task {i} completed")
        ...
        if not check_result['should_complete']:
            ...
            if plan['status'] == 'running':
                result["notes"].append("✓ Plan status remains 'running'")
            else:
...
def test_3_mixed_log_patterns(conn) -> dict:
    ...
    try:
        ...
        for i, pattern in enumerate(patterns, 1):
            time.sleep(0.5)
            add_plan_log(conn, plan_id, f"task-{i}", pattern)
...
def test_4_plan_log_retrieval(conn) -> dict:
    """Test 4: Verify plan logs are stored and retrieved correctly"""
    log("\n" + "=" * 60)
    log("TEST 4: Plan Log Storage and Retrieval")
    log("=" * 60)

    result = {
        "test_id": "E2E-004",
        "test_name": "Log Storage",
        "status": "pending",
        "notes": []
    }
    ...
    try:
        # Create plan
        plan_id = create_test_plan(conn, 3)
        result["notes"].append(f"Created plan {plan_id}")

        # Add various logs
        logs = [
            ("task-1", "info", "Starting task 1"),
            ("task-1", "info", "Working on task 1"),
            ("task-1", "info", "✔ finished — end_turn"),
            ("task-2", "info", "Starting task 2"),
            ("task-2", "info", "Task 2 completed")
        ]

        for task_id, level, message in logs:
            time.sleep(0.2)
            add_plan_log(conn, plan_id, task_id, message)

        result["notes"].append(f"Added {len(logs)} log entries")

        # Retrieve logs
        retrieved_logs = get_plan_logs(conn, plan_id)
        result["notes"].append(f"Retrieved {len(retrieved_logs)} logs from database")

        # Verify logs
        if len(retrieved_logs) == len(logs):
            result["status"] = "passed"
            result["notes"].append("✓ All logs stored and retrieved correctly")

            # Check completion pattern detection
            completion_logs = sum(1 for log in retrieved_logs if check_completion_pattern(log['message']))
            result["notes"].append(f"✓ Detected {completion_logs} completion logs")
        else:
            result["status"] = "failed"
            result["notes"].append(f"✗ Expected {len(logs)} logs, got {len(retrieved_logs)}")
...
def test_5_completion_detection(conn) -> dict:
    """Test 5: Verify completion detection logic"""
    log("\n" + "=" * 60)
    log("TEST 5: Completion Detection Logic")
    log("=" * 60)

    result = {
        "test_id": "E2E-005",
        "test_name": "Completion Detection",
        "status": "pending",
        "notes": []
    }
    ...
    try:
        ...
        plan_id = create_test_plan(conn, 4)
        result["notes"].append(f"Created plan {plan_id} with 4 tasks")

        # Add completion logs for all tasks
        for i in range(1, 5):
            time.sleep(0.5)
            add_plan_log(conn, plan_id, f"task-{i}", f"Task {i} completed successfully")

        result["notes"].append("Added completion logs for all 4 tasks")

        # Check completion
        check_result = check_completion(conn, plan_id)
        result["notes"].append(
            f"Completion check: {check_result['completed_tasks']}/{check_result['total_tasks']} tasks completed"
        )

        if check_result['should_complete']:
            result["status"] = "passed"
            result["notes"].append("✓ Completion detection logic works correctly")
        else:
            result["status"] = "failed"
            result["notes"].append("✗ Completion detection failed")
...
def main():
    ...
    try:
        ...
        test_results.append(test_4_plan_log_retrieval(conn))
        test_results.append(test_5_completion_detection(conn))
