...
/**
 * Agent Linking Comprehensive Integration Test
 *
 * @description Comprehensive tests for agent linking functionality including
 *              auto-link on creation, auto-unlink on deletion, database integrity,
 ...
 * - Projects directory must exist
 *
 * @usage
 *   node tests/integration/agents/agent-linking.test.mjs
 *   OR from project root: node tests/integration/agents/agent-linking.test.mjs
 *
 * @coverage
 * - Agent name display logic and path extraction
 * - Auto-link on agent creation
 * - Auto-unlink on agent deletion
 * - Database integrity and orphaned record detection
...
const DB_PATH = path.join(PROJECT_ROOT, 'api', 'data', 'database.db');
...
try {
  ...
  // Check if project_agents table exists
  const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='project_agents'").all();
  if (tables.length > 0) {
    testPass('project_agents table exists');
  } else {
    testFail('project_agents table does not exist');
  }
  ...
  // Verify agent names in paths
  let nameIssues = 0;
  agents.forEach(agent => {
    const pathParts = agent.workspace_path.split('/');
    const name = pathParts[pathParts.length - 1];
    if (name.startsWith('agent-') || name === 'agents') {
      console.log(`   ⚠️  Issue with name: ${agent.workspace_path}`);
      nameIssues++;
    }
  });

  if (nameIssues === 0) {
    testPass('Agent names appear correct in database paths');
  } else {
    testFail(`Found ${nameIssues} paths with naming issues`);
  }
...
try {
  // Check if create-workspace route includes project linking
  const workspacesRoute = fs.readFileSync(path.join(PROJECT_ROOT, 'api/src/routes/workspaces.ts'), 'utf8');

  if (workspacesRoute.includes('INSERT INTO project_agents')) {
